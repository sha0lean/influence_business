...
function NavBarInternBorder() {
	...
	useEffect(() => {
		axios.post(api.url + "/getProfilPicture", { "token": token }, {
			timeout: 2000,
			headers: {
		...
		axios.post(api.url + "/getUsername", { "token": token }, {
			timeout: 2000,
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json;charset=UTF-8",
		...
		axios.post(api.url + "/getModules", { "token": token }, {
			timeout: 2000,
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json;charset=UTF-8",
		...
		<div id="mainContainerNavInternBorder">
			<div id="containerNavInternBorder">
				<nav>
					...
					<h2>{username}</h2>
					{role === "entrepreneur" &&
						<div class="containerButtonDashboard">
							<Link to="/profilEntrepreneur">Tableau de bord</Link>
						</div>
					}
					{role === "expert" &&
						<div class="containerButtonDashboard">
